...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(Space))==(spaceXAxis,spaceYAxis,space,asteroids,emptyRegion,homebase,starbase);
  Inherited_List_Constants(Machine(Space))==(?);
  List_Constants(Machine(Space))==(spaceXAxis,spaceYAxis,space,asteroids,emptyRegion,homebase,starbase)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(Space))==(spaceXAxis <: NAT1 & spaceXAxis = 1..12 & spaceYAxis <: NAT1 & spaceYAxis = 1..7 & space = spaceXAxis*spaceYAxis & asteroids <: space & asteroids = {7|->7,8|->3,10|->6,11|->2,12|->5,3|->2,3|->5,5|->4,6|->7,7|->1,7|->5} & emptyRegion <: space & emptyRegion/\asteroids = {} & emptyRegion\/asteroids = space & homebase: space & homebase: emptyRegion & homebase = 1|->1 & starbase: space & starbase: emptyRegion & starbase = 6|->4)
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(Space)) == (Type(spaceXAxis) == Cst(SetOf(btype(INTEGER,"[spaceXAxis","]spaceXAxis")));Type(spaceYAxis) == Cst(SetOf(btype(INTEGER,"[spaceYAxis","]spaceYAxis")));Type(space) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(asteroids) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(emptyRegion) == Cst(SetOf(btype(INTEGER,?,?)*btype(INTEGER,?,?)));Type(homebase) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?));Type(starbase) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
